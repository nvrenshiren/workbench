...
- **`scan`** `[--actor]` —— 全量扫描 docs + codeRoots 登记所有产物,按 kind 层级推导 DAG 边(**对账式**:残留的推导边随坐标事实清理,手动声明的边永不动);代码目录级登记(不逐文件)。改了 `moduleMapping` / `kinds` 覆盖等 config 后,**已登记行的坐标也随之收敛(重挂 `coords_remapped`,内容 hash 不变故审批不失效)**;文件**重命名/移动自动跟随**(同内容 hash 且原路径消失的唯一候选 → 保 id 更新路径,审批与关系存活,`auto_moved` 留痕)。*场景:* 批量落地文件后一次入库;代码写完不用手动 `output`,scan 自动维护;调整归并/kind 规则后重跑即收敛旧行;git post-commit 也自动跑。
